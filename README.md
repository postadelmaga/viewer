...
- **Modelli 3D** (`.obj .gltf .glb`) renderizzati su GPU (OpenGL/glow) con **camera orbitale**: trascina per ruotare, rotella per lo zoom, **Adatta** per inquadrare. Materiali/texture ignorati (solo geometria, illuminazione Lambert a faro). Decodifica via `tobj` (OBJ) e `gltf` (glTF 2.0, scena di default appiattita in world-space). Modulo opzionale: feature `mesh` di `viewer-core` (vedi sotto).
